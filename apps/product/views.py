...
class ListProductsView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if not (sortBy == 'date_created' or sortBy == 'valor' or sortBy == 'sold' or sortBy == 'name'):
            sortBy = 'date_created'
    ...
    def post(self, request, format=None):
        ...
        if len(search) == 0:
            ...
            search_results = Product.objects.filter(
                Q(descripcion__icontains=search) | Q(name__icontains=search)
            )
...
class ListBySearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not (sort_by == 'date_created' or sort_by == 'valor' or sort_by == 'sold' or sort_by == 'name'):
        ...
        if price_range == '1 - 19':
            product_results = product_results.filter(valor__gte=1, valor__lt=20)
        elif price_range == '20 - 39':
            product_results = product_results.filter(valor__gte=20, valor__lt=40)
        elif price_range == '40 - 59':
            product_results = product_results.filter(valor__gte=40, valor__lt=60)
        elif price_range == '60 - 79':
            product_results = product_results.filter(valor__gte=60, valor__lt=80)
        elif price_range == 'More than 80':
            product_results = product_results.filter(valor__gte=80)
